...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process a video using MoveNet to track the horizontal center of the scene. "
                    "Centers are saved per frame (with new scene markers), then smoothed using an inverted filter "
                    "to reduce jitter while allowing large movements. Debug mode overlays center dots, scene change text, and keypoint markers."
    )
    parser.add_argument("-i", "--input", help="Path to the input video file")
    parser.add_argument("-o", "--output", default="output_video.mp4", help="Path to the output video file")
    parser.add_argument("-d", "--debug", action="store_true", help="Enable debugging overlays")
    args = parser.parse_args()

    input_video = args.input if args.input else find_default_input()
    if not input_video or not os.path.exists(input_video):
        print("Error: No input file specified and no 'input.*' file found in the current directory.")
        sys.exit(1)

    print(f"Processing video: {input_video}")
    process_video(input_video, args.output, debug=args.debug)